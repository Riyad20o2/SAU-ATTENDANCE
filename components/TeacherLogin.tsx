...
import { User, Lock, ArrowRight, Mail, BookOpen, Camera, Upload, ChevronLeft, Eye, EyeOff } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (teacher) {
            // Save last teacher for Welcome Screen persistence
            localStorage.setItem('sau_last_teacher', JSON.stringify(teacher));
            localStorage.removeItem('sau_last_student');
            onLogin(teacher);
        }
      ...
      <div className="glass-panel w-full p-6 md:p-8 rounded-2xl shadow-2xl relative overflow-hidden transition-all duration-500">
        ...
        <form onSubmit={!isLogin && step === 1 ? handleNextStep : handleSubmit} className="space-y-4 relative z-10">
                  ...
                  <div className="space-y-1 animate-fade-in">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="••••••••"
                        className={`w-full pl-10 pr-4 py-3 rounded-xl bg-[#0f2846]/50 border ${confirmPassword && password !== confirmPassword ? 'border-red-500/50' : 'border-orange-500/10'} text-white placeholder-slate-500 focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none transition-all`}
                      />
                    </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-bold py-3.5 rounded-xl shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-2 mt-4 group relative overflow-hidden"
          >
            {isLoading && !error ? (
