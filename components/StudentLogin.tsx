...
import { User, Lock, ArrowRight, Mail, Camera, Upload, ChevronLeft, Eye, EyeOff } from 'lucide-react';
import { StudentProfile } from '../types';
import Logo from './Logo';
import { loginStudent, registerStudent, loginStudentWithGoogle, loginAdmin, resendVerificationEmail } from '../services/firebase';
...
const StudentLogin: React.FC<StudentLoginProps> = ({ onLogin, onBack }) => {
  ...
  const [academicStage, setAcademicStage] = useState<'First Year' | 'Second Year' | 'Third Year' | 'Fourth Year'>('First Year');
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 2000000) { // 2MB limit for base64 strings in Firestore to prevent quota issues
  ...
  const handleNextStep = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !email || !password || (!isLogin && !department)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // If loginStudent throws, it goes to catch block.
        // If it returns a user, we proceed.
        
        if (user) {
          // Save last student for Welcome Screen persistence
          localStorage.setItem('sau_last_student', JSON.stringify(user));
          localStorage.removeItem('sau_last_teacher');
          onLogin(user);
        }
        ...
        const generatedId = 'S' + Math.floor(Math.random() * 90000 + 10000);
  ...
  if (viewMode === 'VERIFICATION_SENT') {
    return (
      <div className="flex flex-col items-center justify-center w-full max-w-md mx-auto animate-scale-in">
        <div className="glass-panel w-full p-8 rounded-2xl shadow-2xl text-center space-y-6">
          <div className="w-20 h-20 bg-blue-500/20 rounded-full flex items-center justify-center mx-auto border border-blue-500/30 animate-bounce">
          ...
          <div className="bg-[#0f2846]/50 p-4 rounded-xl border border-white/5 text-xs text-slate-500 text-left">
            <p className="font-bold text-slate-400 mb-1">Didn't see the email?</p>
            <ul className="list-disc ml-4 space-y-1">
              <li>Check your spam or junk folder.</li>
              <li>Ensure the email address is correct.</li>
              <li>Wait a few minutes for the delivery.</li>
            </ul>
          </div>
          <button 
    ...
    <div className="flex flex-col items-center justify-center w-full max-w-md mx-auto animate-fade-in-up">
      <div className="glass-panel w-full p-6 md:p-8 rounded-2xl shadow-2xl relative overflow-hidden transition-all duration-500">
        ...
        <div className="flex flex-col items-center mb-6 relative z-10">
          <div className="w-16 h-16 md:w-20 md:h-20 bg-white rounded-full flex items-center justify-center shadow-lg mb-4 ring-4 ring-white/5 overflow-hidden border border-white/10">
             <Logo />
          </div>
        ...
        {error && (
          <div className="mb-4 p-3 rounded-lg bg-red-500/20 border border-red-500/30 text-red-200 text-sm text-center relative z-10 animate-shake">
            {error}
        ...
        {successMsg && (
          <div className="mb-4 p-3 rounded-lg bg-green-500/20 border border-green-500/30 text-green-200 text-sm text-center relative z-10 animate-fade-in">
            {successMsg}
          </div>
        )}
        ...
        <form onSubmit={!isLogin && step === 1 ? handleNextStep : handleSubmit} className="space-y-4 relative z-10">
              ...
              {!isLogin && (
                <>
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-3 rounded-xl bg-[#0f2846]/50 border border-white/10 text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 rounded-xl bg-[#0f2846]/50 border border-white/10 text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider ml-1">Academic Stage</label>
                      <select
                        value={academicStage}
                        onChange={(e) => setAcademicStage(e.target.value as any)}
                        className="w-full px-4 py-3 rounded-xl bg-[#0f2846]/50 border border-white/10 text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all appearance-none"
                      >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider ml-1">Study Type</label>
                      <select
                        value={studyType}
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="student@college.edu.iq"
                    className="w-full pl-10 pr-4 py-3 rounded-xl bg-[#0f2846]/50 border border-white/10 text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                  />
                </div>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full pl-10 pr-12 py-3 rounded-xl bg-[#0f2846]/50 border border-white/10 text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                  />
                  <button 
                    type="button"
                ...
                <div className="space-y-1 animate-fade-in">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="••••••••"
                      className={`w-full pl-10 pr-4 py-3 rounded-xl bg-[#0f2846]/50 border ${confirmPassword && password !== confirmPassword ? 'border-red-500/50' : 'border-white/10'} text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all`}
                    />
                  </div>
          ...
          {!isLogin && step === 2 && (
            <div className="flex flex-col items-center justify-center space-y-6 animate-scale-in py-4">
              <input 
                type="file" 
                ref={fileInputRef} 
              ...
              <div 
                ...
                className="w-40 h-40 rounded-full border-4 border-dashed border-slate-600 hover:border-blue-500 hover:bg-white/5 flex items-center justify-center cursor-pointer transition-all group relative overflow-hidden bg-[#0f2846]"
              >
                {profileImage ? (
              ...
              <div className="text-center">
                <p className="text-sm text-slate-300">Choose a clear photo for your ID.</p>
                <button 
                   type="button" 
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-3.5 rounded-xl shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-2 mt-4 group relative overflow-hidden"
          >
            {isLoading && !error ? (
        ...
        {isLogin && (
          <div className="mt-4 relative z-10">
            ...
            <button
                ...
                disabled={isLoading}
                className="w-full bg-white hover:bg-slate-100 text-slate-800 font-semibold py-3 rounded-xl transition-all active:scale-[0.98] flex items-center justify-center gap-3 mt-2 disabled:opacity-70"
            >
                {isLoading ? (
        ...
        <div className="mt-6 text-center space-y-4 relative z-10">
          <p className="text-sm text-slate-400">
            ...
            <button 
              onClick={toggleMode}
              className="ml-2 text-blue-400 hover:text-blue-300 font-semibold underline decoration-dotted underline-offset-4 transition-colors"
            >
